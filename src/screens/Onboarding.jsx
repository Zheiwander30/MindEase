    ...
    <div className="flex min-h-screen flex-col items-center justify-center bg-app-bg px-6 py-10">
      ...
      {step === 0 && (
        <div className="w-full max-w-sm animate-fade-in">
          <h1 className="mb-1 text-center text-2xl font-extrabold text-app-text">Welcome to MindEase 💜</h1>
          <p className="mb-8 text-center text-sm text-app-muted">Your personal mental wellness companion. Let us get you set up.</p>
          <Card>
            <label htmlFor="onboard-name" className="mb-1 block text-sm font-bold text-app-text">
              What should we call you?
            </label>
            <p className="mb-3 text-xs text-app-muted">This will appear on your home screen and profile.</p>
            <input
              id="onboard-name"
              type="text"
            ...
            />
            {nameError && <p className="mt-2 text-xs text-mood-overwhelmed">{nameError}</p>}
          </Card>
      ...
      {step === 1 && (
        <div className="w-full max-w-sm animate-fade-in">
          <h1 className="mb-1 text-center text-2xl font-extrabold text-app-text">Hi, {name}! 👋</h1>
          <p className="mb-8 text-center text-sm text-app-muted">How would you describe yourself right now? (Optional)</p>
          <div className="flex flex-col gap-2">
            {STATUS_OPTIONS.map((opt) => (
              <button key={opt.value} type="button" onClick={() => setStatus(opt.value)} aria-pressed={status === opt.value}
                className={`rounded-2xl border px-4 py-3 text-left text-sm font-medium transition-colors ${
                  status === opt.value ? 'border-brand bg-brand text-white' : 'border-app-border bg-app-card text-app-text'}`}>
                {opt.label}
              </button>
            ))}
            <button type="button" onClick={() => setStatus('__custom__')} aria-pressed={status === '__custom__'}
              className={`rounded-2xl border px-4 py-3 text-left text-sm font-medium transition-colors ${
                status === '__custom__' ? 'border-brand bg-brand/10 text-brand' : 'border-app-border bg-app-card text-app-text'}`}>
              ✏️ Write my own…
            </button>
            {status === '__custom__' && (
              <input type="text" value={customStatus} onChange={(e) => setCustomStatus(e.target.value)}
                placeholder="Describe yourself…" maxLength={60} autoFocus
                autoComplete="off"
          ...
          <button type="button" onClick={handleFinish} className="mt-2 w-full py-2 text-xs text-app-muted">
            Skip for now
          </button>
        </div>
      )}
